# Tensorflow Job Using a Software Module

[![](/Images/Download-Button.png)](tensorflow-module-job.tar.gz)

## About 
This script makes use of a Tensorflow image available as a module on HPC to execute a workflow on a GPU node. 

We keep reasonably up-to-date Nvidia tensorflow images available on the system as modules. When you load a tensorflow module, it adds a shortcut to your environment that points to the relevant image, for example:

```console
[netid@i16n0 ~]$ module avail tensorflow

---------------------------------------- /opt/ohpc/pub/modulefiles ----------------------------------------
   tensorflow/nvidia/2.0.0    tensorflow/nvidia/2.6.0    tensorflow/nvidia/2.9.1 (D)
   
[netid@i16n0 ~]$ module load tensorflow/nvidia/2.0.0
[netid@i16n0 ~]$ type tensorflow
...
echo
echo "Executing: tensorflow tensorflow-example.py"
echo 
tensorflow tensorflow-example.py
```
...
## Job Submission
```console
[netid@i16n0 ~]$ sbatch tensorflow_example.slurm 
Submitted batch job 1471449
```

## Output
```
[netid@i16n0 ~]$ cat Sample-tensorflow-example-1471449.out 
